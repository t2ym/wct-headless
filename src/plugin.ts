...
const plugin: wct.PluginInterface = (
  ...
  const onConfigure = async () => {
    ...
    const expanded = await browsers.expand(names, pluginOptions.chromeOptions);

    wct.emit(
        'log:debug',
        'Expanded local browsers:', names, 'into capabilities:', expanded, 'with chromeOptions:', pluginOptions.chromeOptions);
